...
<body>
  ...
  <table>
    ...
    def save_csv(self, path: str) -> str:
        ...
        for i, f in enumerate(self.findings, 1):
            w.writerow([i, f.url, f.param, f.severity, f.xss_type, f.context,
                        f.payload, f.encoding_used,
                        f.waf_bypassed, f.verified, f.evidence[:200]])
    ...
    def save_md(self, path: str) -> str:
        ...
        if not self.findings:
            ...
            for i, f in enumerate(self.findings, 1):
                lines.append(
                    f"| {i} | `{f.url[:60]}` | `{f.param}` | **{f.severity}** "
    ...
    def save_sarif(self, path: str) -> str:
        ...
        for f in self.findings:
            results.append({
                ...
                "message": {
                    "text": (
                        f"{f.xss_type.title()} XSS in parameter '{f.param}' "
                        f"(context: {f.context}). "
