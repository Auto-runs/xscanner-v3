...
class TestFuzzyDetector:
    ...
    def test_detects_new_executable_tags(self):
        ...
        response = "<html><body><p>test</p><script>evil()</script></body></html>"
        result   = d.analyze("irrelevant_payload", baseline, response)
    ...
    def test_fuzzy_catches_partial_match(self):
        ...
        # Response has encoded version of payload
        response = "<html><body>&lt;script&gt;alert(1)&lt;/script&gt;</body></html>"
        result   = d.analyze(payload, baseline, response)
        # Token overlap should detect "alert" keyword
    ...
    def test_entropy_delta_detected(self):
        ...
        d = FuzzyDetector()
        baseline = "a" * 1000
        response = "a" * 500 + "bcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()" * 20
        result   = d.analyze("payload", baseline, response)
    ...
    def test_structural_change_detected(self):
        ...
        d = FuzzyDetector()
        baseline = "<p>hi</p>"
        # Significantly longer response
        response = "<p>hi</p>" + "<div>" * 100 + "content" + "</div>" * 100
        result   = d.analyze("x", baseline, response)
